...
double Round(double num, int nCasas)
{
	...
	// Multiplicamos para que fique com o número de casas
	// decimais desejado exatamente na vírgula:
	if(nCasas != 0)
	    num *= pow(10,nCasas);

	// Isolamos o número que determinará o arredondamento:
	double dInt, dFrac;
	dFrac = modf(num,&dInt);
	...
	{
		// Se não for par, somamos um para que fique.
		if( ((__int64)dInt)%2 == 1)
			dRet = dInt+1;
		else
			dRet = dInt;
...
int Significativos(double& grandeza, double& incerteza, bool cientifico)
{
	...
	// Reduzimos os números á ordem zero ( 1 <= x < 10 )
    double grd0 = grandeza  / dExp10;
    if(fabs(grd0) == numeric_limits<double>::infinity())
	{
		// Passamos a ignorar a incerteza, pois está gerando erro
		// Bateu nos limites.
