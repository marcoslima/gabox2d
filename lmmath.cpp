...
{
	// Quantas casas decimais tem este número
	int nCasas = 0;
	double dInt, dFrac;
	while(true)
	{
...
{
	// Pegamos o MENOR desses números:
	...
	{
		bComum = true;
		// Cada divisor que dividir todos os números, avançamos:
		for(it = numeros.begin(); it!= numeros.end(); it++)
		{
			if(*it%div != 0)
			{
				// Este divisor já não é comum. Avançamos:
				// Descartamos: este divisor não divide um deles:
				div++;
				bComum = false;
				break;
			}
		...
		// Se todos eles foram divisíveis por este divisor, entra no comum:
		if(bComum)
		{
			lComum *= div;
...
// Obtém o termo geral de um polinomio da forma (a+b)^n
...
#if 0 // Abordagem velha
{
	...
	{
		// Cada elemento da primeira fatoração será procurado
		// nos elementos das demais fatorações.
		// Se for achado em todas, ele é um comum

		for(i = 1; i < nSize; i++)
		{
			if(!binary_search(vecFatorados[i].begin(),vecFatorados[i].end(),vecFatorados[0][k]))
			{
				// Não é comum. Podemos passar para o próximo:
			}
