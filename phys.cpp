...
namespace PHYS
{
    ...
    void _make_wall(const b2WorldId worldId, const b2Vec2 pos, const b2Vec2 size)
    {
        ...
        const b2BodyId wallId = b2CreateBody(worldId, &wallDef);
    ...
    void _make_left_wall(const b2WorldId worldId, const MODEL::CEnv &env, const float tick)
    {
        const auto pos = b2Vec2(env._tlx - tick, (env._bry + env._tly) / 2);
        const auto size = b2Vec2(tick, fabs(env._bry - env._tly));
        _make_wall(worldId, pos, size);
    }

    void _make_right_wall(const b2WorldId worldId, const MODEL::CEnv &env, const float tick)
    {
    ...
    void _make_ceiling(const b2WorldId worldId, const MODEL::CEnv &env, const float tick)
    {
    ...
    void _create_ground(CWorld &world, const MODEL::CEnv &env)
    {
        ...
        _make_left_wall(world.m_WorldId, env, tick);
        _make_right_wall(world.m_WorldId, env, tick);
        _make_ceiling(world.m_WorldId, env, tick);
    }
